...
function BlogLargeRightSidebar() {
  ...
  const fetchBlogContents = async () => {
    try {
      const res = await fetch('http://localhost:1337/api/blogcontents?populate=*', {
        headers: {
  ...
  const filteredBlogs = blogs.filter((blog) => {
    const fieldToSearch = blog.attributes.blogFieldToSearch?.toLowerCase() || "";
            ...
            <div className="row">
              <div className="col-xl-8 col-lg-8 m-b50">
                {currentBlogs.map((blog) => (
                  <div className="dlab-blog style-1 bg-white text-center m-b50" key={blog.id}>
                ...
                <nav aria-label="Blog Pagination">
                  <ul className="pagination text-center p-t20">
                    <li className={`page-item ${currentPage === 1 ? "disabled" : ""}`}>
                      <Link href="#"><a className="page-link prev" onClick={() => handlePageChange(currentPage - 1)}>Prev</a></Link>
                    </li>
                   
                    <li className={`page-item ${currentPage === totalPages ? "disabled" : ""}`}>
                      <Link href="#"><a className="page-link next" onClick={() => handlePageChange(currentPage + 1)}>Next</a></Link>
                    </li>
                  </ul>
                </nav>
              </div>
...
/>
                  ...
                  <div className="widget recent-posts-entry">
					...
					{currentBlogs.slice(0, 4).map((blog) => (
                        ...
                        width={400}
                        height={270}
                        src={blog.attributes.blogphotos.data.attributes.formats.small.url}
                        alt={blog.attributes.heading}
                      />
                        </div>
                        <div className="dlab-info">
                          <h4 className="title"><a href="/blog-details-1">{blog.attributes.heading}</a></h4>
                          <div className="dlab-meta">
                            <ul>
                              <li className="post-date"> {blog.attributes.Date}</li>
                            </ul>
                          </div>
                        </div>
                      </div>
